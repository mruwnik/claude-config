...
SESSION_LOG_FILE = Path("/var/log/claude/session.log")
...
def log_session_message(message: str) -> None:
    """Append a timestamped message to the session log."""
    try:
        timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S+00:00")
        with SESSION_LOG_FILE.open("a") as f:
...
def check_repeated_block(reason: str) -> bool:
    ...
    if len(history) >= MAX_REPEATED_BLOCKS and len(set(history)) == 1:
        ...
        # Reset history so next session starts fresh
        try:
...
def poll_for_comments(session_id: str, pr_info: str, repo_path: str) -> None:
    ...
    while True:
        ...
        if pr_status in ["merged", "closed"]:
            ...
            log_session_message(f"PR review hook: {pr_info} has been {pr_status}. Requesting session termination.")
            block(
                f"ACTION REQUIRED: End this session - {pr_info} has been {pr_status}.\n\n"
                "The Claude instance is running in tmux. To properly shut down:\n"
                "1. Run: `tmux kill-server`\n\n"
                "This will terminate all tmux sessions and end the Claude Code process."
            )

        if time.time() - idle_start >= MAX_IDLE:
            print("12h idle. Ending.", file=sys.stderr)
            log_session_message(f"PR review hook: 12h idle timeout reached for {pr_info}. Requesting session termination.")
            block(
                f"ACTION REQUIRED: End this session - 12 hour idle timeout reached for {pr_info}.\n\n"
                "The Claude instance is running in tmux. To properly shut down:\n"
                "1. Run: `tmux kill-server`\n\n"
                "This will terminate all tmux sessions and end the Claude Code process."
            )
